...
function onOpen() {
  SpreadsheetApp.getUi()
      .createMenu('スライド生成ツール')
      .addItem('起動', 'openWebApp')
      .addToUi();
}
...
return;
  ...
  const html = HtmlService.createHtmlOutput(
    `<p style="font-family: sans-serif;">以下のリンクをクリックしてツールを新しいタブで開いてください。</p>
     <a href="${WEBAPP_URL}" target="_blank" style="font-family: sans-serif; font-size: 14px;">スライド作成ツールを開く</a>`
  )
  .setWidth(400)
  .setHeight(120);
...
/**
 * Webアプリケーションとしてアクセスされたときに呼ばれるメイン関数
 */
function doGet(e) {
...
/**
 * ========================================
 * プリセット管理関数 (スプレッドシート連携)
 * ========================================
 */

// 設定キーとスプレッドシートのヘッダーのマッピング
const SETTINGS_KEY_TO_HEADER = {
  primaryColor: 'プライマリカラー',
  fontFamily: 'フォント',
  footerText: 'フッターテキスト',
  driveFolderUrl: '保存先フォルダURL',
  showTitleUnderline: 'タイトルアンダーライン',
  showBottomBar: 'フッターバー',
  showDateColumn: '日付表示',
  enableGradient: 'グラデーション適用',
  gradientStart: 'グラデーション開始色',
  gradientEnd: 'グラデーション終了色',
  headerLogoUrl: 'ヘッダーロゴ',
  closingLogoUrl: 'クロージングロゴ',
  titleBgUrl: 'タイトル背景',
  sectionBgUrl: 'セクション背景',
  mainBgUrl: 'メイン背景',
...
/**
 * スプレッドシートからプリセットを取得してWebアプリに返す
 * @returns {Object} プリセットのオブジェクト
 */
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      headers.forEach((header, colIndex) => {
        ...
        if (key) {
          ...
          if (['showTitleUnderline', 'showBottomBar', 'showDateColumn', 'enableGradient'].includes(key)) {
...
/**
 * プリセットをスプレッドシートに保存（新規または上書き）
 * @param {string} presetName - 保存するプリセット名
...
function savePresetToSheet(presetName, settings) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = ss.getSheetByName(PRESET_SHEET_NAME);
    if (!sheet) throw new Error(`シート "${PRESET_SHEET_NAME}" が見つかりません。`);
    
    const data = sheet.getDataRange().getValues();
    const headers = data.length > 0 ? data[0] : [];
    const nameColIndex = headers.indexOf(PRESET_NAME_HEADER);
    if (nameColIndex === -1) throw new Error(`スプレッドシートに "${PRESET_NAME_HEADER}" 列が必要です。`);
